#!/usr/bin/env python3
"""
Patagonia ESG Analysis - PDF Text Extraction using PyMuPDF (robust version)
...
try:
    ...
    os.system("pip install --user PyMuPDF pandas openpyxl")
    import fitz
    import pandas as pd


class PatagoniaTextExtractor:
    """Extract text from Patagonia PDFs with precise page number tracking using PyMuPDF"""
    ...
    def extract_with_pymupdf(self, pdf_path: str) -> List[Dict]:
        """
        Extract text using PyMuPDF (fitz) - robust and fast
    ...
    def create_master_index(self, all_documents: List[Dict]):
        ...
        print(f"✓ Master index Excel saved: {excel_path}")
